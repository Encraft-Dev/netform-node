...
router.post('/add',function(req,res,next){
  ...
  valid?users.addUser(data,"test"):false
  res.send(output);
...
router.get('/:id/:time',function(req,res,next){
  //list all users in current day
  var simid = "test" //Date.today().toString("yyyy_MM_dd")
  var data = JSON.parse(zlib.unzipSync(fs.readFileSync(path.join(appDir,"results",simid,"veh",req.params.time+".json.gz"))));
...
router.get('/:id',function(req,res,next){
  //list all users in current day
  var simid = "test" //Date.today().toString("yyyy_MM_dd")
  var data = JSON.parse(fs.readFileSync(path.join(appDir,"results",simid,"users.json")));
  var userid = req.params.id
  console.log(userid)
  // check for user existing

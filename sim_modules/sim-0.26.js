var Sim = exports.Sim = function () { this.simTime = 0;
    this.entities = [];
...
Sim.prototype.addEntity = function(a) {
    ...
    if (!a.time) a.time = function() {
            return this.sim.time() }, a.setTimer = function(a) { a = new Sim.Request(this, this.sim.time(), this.sim.time() + a);
            this.sim.queue.insert(a);
            return a }, a.waitEvent = function(a) {
            var b = new Sim.Request(this, this.sim.time(), 0);
            b.source = a;
            a.addWaitList(b);
            return b }, a.queueEvent = function(a) {
            var b = new Sim.Request(this, this.sim.time(), 0);
            b.source = a;
            a.addQueue(b);
            return b }, a.useFacility =
        function(a, b) {
            var c = new Sim.Request(this, this.sim.time(), 0);
            c.source = a;
            a.use(b, c);
            return c }, a.putBuffer = function(a, b) {
            var c = new Sim.Request(this, this.sim.time(), 0);
            c.source = a;
            a.put(b, c);
            return c }, a.getBuffer = function(a, b) {
            var c = new Sim.Request(this, this.sim.time(), 0);
            c.source = a;
            a.get(b, c);
            return c }, a.putStore = function(a, b) {
            var c = new Sim.Request(this, this.sim.time(), 0);
            c.source = a;
            a.put(b, c);
            return c }, a.getStore = function(a, b) {
            var c = new Sim.Request(this, this.sim.time(), 0);
            c.source = a;
            a.get(b, c);
            return c },
        a.send = function(a, b, c) { b = new Sim.Request(this.sim, this.time(), this.time() + b);
            b.source = this;
            b.msg = a;
            b.data = c;
            b.deliver = this.sim.sendMessage;
            this.sim.queue.insert(b) }, a.log = function(a) { this.sim.log(a, this) };
    var b = function(a) {
        function b() {}
...
Sim.Facility.prototype.reset = function() { this.queue.reset();
    this.stats.reset();
    this.busyDuration = 0 };
...
Sim.Facility.prototype.finalize = function(a) { this.stats.finalize(a);
    ...
    for (; this.free > 0 && !this.queue.empty();) {
        ...
        if (!b.cancelled) {
            for (var c = 0; c < this.freeServers.length; c++)
                if (this.freeServers[c]) { this.freeServers[c] = !1;
                    b.msg = c;
                    break }
            this.free--;
...
Sim.Facility.prototype.useFCFSCallback = function(a) { this.free++;
...
Sim.Facility.prototype.useProcessorSharing = function(a, b) { b.duration = a;
    b.cancelRenegeClauses();
...
Sim.Buffer = function(a, b, c) { this.name = a;
    this.capacity = b;
...
Sim.Buffer.prototype.get = function(a, b) { this.getQueue.empty() && a <= this.available ? (this.available -= a, b.deliverAt = b.entity.time(), b.entity.sim.queue.insert(b), this.getQueue.passby(b.deliverAt), this.progressPutQueue()) : (b.amount = a, this.getQueue.push(b, b.entity.time())) };
...
Sim.Store = function(a, b) { this.name = a;
    this.capacity = b;
    this.objects = [];
    this.putQueue = new Sim.Queue;
    this.getQueue = new Sim.Queue };
    ...
    if (this.getQueue.empty() && this.current() > 0) {
        ...
        if (a)
            for (var f = 0; f < this.objects.length; f++) {
                if (d = this.objects[f], a(d)) { c = !0;
                    this.objects.splice(f, 1);
                    break } } else d = this.objects.shift(), c = !0;
        if (c) { this.available--;
            b.msg = d;
...
Sim.Store.prototype.getStats = function() {
    return this.getQueue.stats };
Sim.Event = function(a) { this.name = a;
    this.waitList = [];
    this.queue = [];
    this.isFired = !1 };
Sim.Event.prototype.addWaitList = function(a) { this.isFired ? (a.deliverAt = a.entity.time(), a.entity.sim.queue.insert(a)) : this.waitList.push(a) };
Sim.Event.prototype.addQueue = function(a) { this.isFired ? (a.deliverAt = a.entity.time(), a.entity.sim.queue.insert(a)) : this.queue.push(a) };
...
Sim.Event.prototype.clear = function() { this.isFired = !1 };
Sim.Request = function(a, b, c) { this.entity = a;
    this.scheduledAt = b;
    this.deliverAt = c;
    this.callbacks = [];
    this.cancelled = !1;
...
Sim.Request.prototype.done = function(a, b, c) { this.callbacks.push([a, b, c]);
    return this };
...
Sim.Request.prototype.unlessEvent = function(a, b, c, d) {
    ...
    if (a instanceof Sim.Event) {
        ...
        f.msg = a;
        a.addWaitList(f) } else if (a instanceof Array)
        for (var e = 0; e < a.length; e++) f = this._addRequest(0, b, c, d), f.msg = a[e], a[e].addWaitList(f);
    return this };
Sim.Request.prototype.setData = function(a) { this.data = a;
    return this };
Sim.Request.prototype.deliver = function() { this.cancelled || (this.cancel(), this.callbacks && (this.group && this.group.length > 0 ? this._doCallback(this.group[0].source, this.msg, this.group[0].data) : this._doCallback(this.source, this.msg, this.data))) };
Sim.Request.prototype.cancelRenegeClauses = function() { this.noRenege = !0;
...
Sim.Request.prototype.Null = function() {
    return this };
Sim.Request.prototype._addRequest = function(a, b, c, d) { a = new Sim.Request(this.entity, this.scheduledAt, a);
...
Sim.Request.prototype._doCallback = function(a, b, c) {
    for (var d = 0; d < this.callbacks.length; d++) {
        ...
        if (f) {
            ...
            e.callbackData = null } } };
Sim.Queue = function(a) { this.name = a;
    this.data = [];
    this.timestamp = [];
    this.stats = new Sim.Population };
...
Sim.Queue.prototype.push = function(a, b) { this.data.push(a);
    this.timestamp.push(b);
    this.stats.enter(b) };
Sim.Queue.prototype.unshift = function(a, b) { this.data.unshift(a);
...
Sim.Queue.prototype.pop = function(a) {
    ...
    return b };
Sim.Queue.prototype.passby = function(a) { this.stats.enter(a);
    this.stats.leave(a, a) };
Sim.Queue.prototype.finalize = function(a) { this.stats.finalize(a) };
Sim.Queue.prototype.reset = function() { this.stats.reset() };
Sim.Queue.prototype.clear = function() { this.reset();
    this.data = [];
    this.timestamp = [] };
...
Sim.PQueue.prototype.remove = function() {
    ...
    if (!(b <= 0)) {
        ...
        a[d] = f;
        return c } };
Sim.DataSeries = function(a) { this.name = a;
    this.reset() };
Sim.DataSeries.prototype.reset = function() { this.Q = this.A = this.W = this.Count = 0;
    this.Max = -Infinity;
    this.Min = Infinity;
    this.Sum = 0;
    if (this.histogram)
        for (var a = 0; a < this.histogram.length; a++) this.histogram[a] = 0 };
Sim.DataSeries.prototype.setHistogram = function(a, b, c) { this.hLower = a;
    this.hUpper = b;
...
Sim.TimeSeries = function(a) { this.dataSeries = new Sim.DataSeries(a) };
Sim.TimeSeries.prototype.reset = function() { this.dataSeries.reset();
    this.lastTimestamp = this.lastValue = NaN };
Sim.TimeSeries.prototype.setHistogram = function(a, b, c) { this.dataSeries.setHistogram(a, b, c) };
Sim.TimeSeries.prototype.getHistogram = function() {
    return this.dataSeries.getHistogram() };
Sim.TimeSeries.prototype.record = function(a, b) { isNaN(this.lastTimestamp) || this.dataSeries.record(this.lastValue, b - this.lastTimestamp);
    this.lastValue = a;
    this.lastTimestamp = b };
Sim.TimeSeries.prototype.finalize = function(a) { this.record(NaN, a) };
...
Sim.Population = function(a) { this.name = a;
    this.population = 0;
    this.sizeSeries = new Sim.TimeSeries;
    this.durationSeries = new Sim.DataSeries };
Sim.Population.prototype.reset = function() { this.sizeSeries.reset();
    this.durationSeries.reset();
    this.population = 0 };
Sim.Population.prototype.enter = function(a) { this.population++;
    this.sizeSeries.record(this.population, a) };
Sim.Population.prototype.leave = function(a, b) { this.population--;
    this.sizeSeries.record(this.population, b);
    this.durationSeries.record(b - a) };
Sim.Population.prototype.current = function() {
    return this.population };
Sim.Population.prototype.finalize = function(a) { this.sizeSeries.finalize(a) };


var Random = exports.Random = function(a) { a = a === void 0 ? (new Date).getTime() : a;
    this.N = 624;
    this.M = 397;
...
Random.prototype.init_genrand = function(a) { this.mt[0] = a >>> 0;
        ...
        for (var c = Math.sqrt(2 * a - 1), d = a - this.LOG4, f = a + c;;) {
            ...
            if (!(e < 1.0E-7 || j > 0.9999999)) {
                var h = 1 - this.random(),
                    ...
                    e = e * e * h,
                    i = d + f * i - g;
                if (i + this.SG_MAGICCONST - 4.5 * e >= 0 || i >= Math.log(e)) return g * b } } else if (a == 1) {
            for (var j = this.random(); j <= 1.0E-7;) j = this.random();
            return -Math.log(j) * b } else {
            for (;;)
